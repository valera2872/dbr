...
check(packageJson.version === '0.6.1', 'package.json должен иметь версию 0.6.1');
check(build.includes("APP_BUILD = 'v0.6.1'"), 'src/build.ts должен объявлять APP_BUILD v0.6.1');
...
check(main.indexOf("./premiumPass'") < main.indexOf("./versionGuard'"), 'versionGuard должен загружаться после premiumPass');
...
check(serviceWorker.includes('dbr-v0-6-1-premium-pass'), 'Service worker не использует cache key Premium Pass v0.6.1');
...
console.log(`\nPremium smoke passed: ${evidenceIds.length + 4} evidence contracts, build ${packageJson.version}.`);
