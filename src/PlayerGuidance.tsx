  ...
  switch (state.derived.stage) {
    ...
    case 'kirill-interrogation':
      return {
        ...
        instruction: 'Вернитесь к Кириллу. Сначала зафиксируйте его версию, затем предъявляйте найденные доказательства в логической связке.',
        progress: `Вопросов: ${Math.min(state.interrogation.asked.length, 3)}/3 · предъявлено: ${state.interrogation.presented.length}`,
        why: 'Теперь у вас достаточно независимых материалов, чтобы проверять не интуицию, а конкретное противоречие.',
    ...
    {onboardingOpen && <div className="player-onboarding-backdrop">
      <section className="player-onboarding" role="dialog" aria-modal="true" aria-label="Как играть в ДБР">
        ...
        <div className="player-onboarding-grid">
          ...
          <article><span>02</span><div><strong>Люди</strong><p>Сверяйте показания с фактами. Новые вопросы появляются по мере расследования.</p></div></article>
